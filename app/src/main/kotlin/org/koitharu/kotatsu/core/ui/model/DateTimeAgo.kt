...
sealed class DateTimeAgo {
    ...
    data class MinutesAgo(val minutes: Int) : DateTimeAgo() {
        override fun format(context: Context): String = context.resources.getQuantityStringSafe(
    ...
    data class MonthsAgo(val months: Int) : DateTimeAgo() {
        override fun format(context: Context): String = if (months == 0) {
    ...
    data class Absolute(private val date: LocalDate) : DateTimeAgo() {
        override fun format(context: Context): String = if (date == EPOCH_DATE) {
