...
class BookmarksRepository @Inject constructor(
    ...
    fun observeBookmark(manga: Manga, chapterId: Long, page: Int): Flow<Bookmark?> = db.getBookmarksDao().observe(manga.id, chapterId, page).map { it?.toBookmark(manga) }

    fun observeBookmarks(manga: Manga): Flow<List<Bookmark>> = db.getBookmarksDao().observe(manga.id).mapItems { it.toBookmark(manga) }

    fun observeBookmarks(): Flow<Map<Manga, List<Bookmark>>> = db.getBookmarksDao().observe().map { map ->
