...
class MangaSearchRepository @Inject constructor(
    ...
    suspend fun getRareTags(source: MangaSource, limit: Int): List<MangaTag> = db.getTagsDao().findRareTags(source.name, limit).toMangaTagsList()

    suspend fun getTopTags(source: MangaSource, limit: Int): List<MangaTag> = db.getTagsDao().findPopularTags(source.name, limit).toMangaTagsList()
    ...
    suspend fun getAuthors(source: MangaSource, limit: Int): List<String> = db.getMangaDao().findAuthorsBySource(source.name, limit)
}
