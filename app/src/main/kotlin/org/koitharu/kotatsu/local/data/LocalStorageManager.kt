...
class LocalStorageManager @Inject constructor(
    ...
    fun isOnExternalStorage(file: File): Boolean = !file.absolutePath.contains(context.packageName)

    fun hasExternalStoragePermission(isReadOnly: Boolean): Boolean = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
    ...
    @WorkerThread
    private fun getFallbackStorageDir(): File? = context.getExternalFilesDir(DIR_NAME) ?: File(context.filesDir, DIR_NAME).takeIf {
        it.exists() || it.mkdirs()
    }
