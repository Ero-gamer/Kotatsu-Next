...
class SourcePresetsRepository @Inject constructor(
    ...
    fun observeAll(): Flow<List<SourcePreset>> = dao.observeAll().mapItems { it.toSourcePreset() }

    fun observe(id: Long): Flow<SourcePreset?> = dao.observe(id).map { it?.toSourcePreset() }

    suspend fun getAll(): List<SourcePreset> = dao.findAll().map { it.toSourcePreset() }

    suspend fun getById(id: Long): SourcePreset? = dao.find(id)?.toSourcePreset()
