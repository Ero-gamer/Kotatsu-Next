...
class MangaSourcesRepository @Inject constructor(
    ...
    fun observeIsEnabled(source: MangaSource): Flow<Boolean> = dao.observeIsEnabled(source.name).onStart { assimilateNewSources() }

    fun observeEnabledSourcesCount(): Flow<Int> = combine(
    ...
    fun observeAvailableSourcesCount(): Flow<Int> = combine(
        observeIsNsfwDisabled(),
        observeAllEnabled().flatMapLatest { isAllSourcesEnabled ->
            dao.observeAll(!isAllSourcesEnabled, SourcesSortOrder.MANUAL)
        },
    ...
    suspend fun isSetupRequired(): Boolean = settings.sourcesVersion == 0 && dao.findAllEnabledNames().isEmpty()
    ...
    private fun observeExternalSources(): Flow<List<ExternalMangaSource>> = callbackFlow {
