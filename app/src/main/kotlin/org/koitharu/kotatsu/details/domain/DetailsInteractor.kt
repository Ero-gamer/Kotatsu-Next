...
class DetailsInteractor @Inject constructor(
    ...
    fun observeNewChapters(mangaId: Long): Flow<Int> = settings.observeAsFlow(AppSettings.KEY_TRACKER_ENABLED) { isTrackerEnabled }
    ...
    fun observeScrobblingInfo(mangaId: Long): Flow<List<ScrobblingInfo>> = combine(
        scrobblers.map { it.observeScrobblingInfo(mangaId) },
    ) { scrobblingInfo ->
        scrobblingInfo.filterNotNull()
    }

    fun observeIncognitoMode(mangaFlow: Flow<Manga?>): Flow<TriStateOption> = mangaFlow
        .filterNotNull()
