...
class HistoryRepository @Inject constructor(
    ...
    fun observeLast(): Flow<Manga?> = db.getHistoryDao().observeAll(1).map {
        val first = it.firstOrNull()
        first?.toManga()
    }

    fun observeAll(): Flow<List<Manga>> = db.getHistoryDao().observeAll().mapItems {
        it.toManga()
    }

    fun observeAll(limit: Int): Flow<List<Manga>> = db.getHistoryDao().observeAll(limit).mapItems {
        it.toManga()
    }
    ...
    fun observeOne(id: Long): Flow<MangaHistory?> = db.getHistoryDao().observe(id).map {
        it?.toMangaHistory()
    }
    ...
    suspend fun getOne(manga: Manga): MangaHistory? = db.getHistoryDao().find(manga.id)?.recoverIfNeeded(manga)?.toMangaHistory()
    ...
    fun observeShouldSkip(manga: Manga): Flow<Boolean> = settings.observe(AppSettings.KEY_INCOGNITO_MODE, AppSettings.KEY_INCOGNITO_NSFW)
