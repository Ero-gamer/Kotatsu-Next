...
abstract class SyncProvider : ContentProvider() {
    ...
    override fun bulkInsert(uri: Uri, values: Array<out ContentValues>): Int = runAtomicTransaction { super.bulkInsert(uri, values) }

    private fun getTableName(uri: Uri): String? = uri.pathSegments.singleOrNull()?.takeIf { it in supportedTables }

    private fun <R> runAtomicTransaction(callable: Callable<R>): R = synchronized(database) {
