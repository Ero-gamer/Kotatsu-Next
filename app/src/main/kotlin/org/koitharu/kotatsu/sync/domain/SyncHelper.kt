...
class SyncHelper @AssistedInject constructor(
    ...
    private fun getHistory(): List<HistorySyncDto> = provider.query(authorityHistory, TABLE_HISTORY).use { cursor ->
    ...
    private fun getFavourites(): List<FavouriteSyncDto> = provider.query(authorityFavourites, TABLE_FAVOURITES).map { cursor ->
        val manga = getManga(authorityFavourites, cursor.getLong(cursor.getColumnIndexOrThrow("manga_id")))
        FavouriteSyncDto(cursor, manga)
    }

    private fun getFavouriteCategories(): List<FavouriteCategorySyncDto> = provider.query(authorityFavourites, TABLE_FAVOURITE_CATEGORIES).map { cursor ->
