...
class FavouritesRepository @Inject constructor(
    ...
    suspend fun findPopularTagTitles(categoryId: Long, limit: Int): List<String> = db.getFavouritesDao().run {
        if (categoryId == 0L) {
    ...
    fun observeCategories(): Flow<List<FavouriteCategory>> = db.getFavouriteCategoriesDao().observeAll().mapItems {
        it.toFavouriteCategory()
    }.distinctUntilChanged()

    fun observeCategoriesForLibrary(): Flow<List<FavouriteCategory>> = db.getFavouriteCategoriesDao().observeAllVisible().mapItems {
        it.toFavouriteCategory()
    }.distinctUntilChanged()

    fun observeCategoriesWithCovers(): Flow<Map<FavouriteCategory, List<Cover>>> = db.invalidationTracker.createFlow(
        TABLE_FAVOURITES,
    ...
    suspend fun getAllFavoritesCovers(order: ListSortOrder, limit: Int): List<Cover> = db.getFavouritesDao().findCovers(order, limit)

    fun observeCategory(id: Long): Flow<FavouriteCategory?> = db.getFavouriteCategoriesDao().observe(id)
        .map { it?.toFavouriteCategory() }

    fun observeCategoriesIds(mangaId: Long): Flow<Set<Long>> = db.getFavouritesDao().observeIds(mangaId).map { it.toSet() }

    fun observeCategories(mangaId: Long): Flow<Set<FavouriteCategory>> = db.getFavouritesDao().observeCategories(mangaId).map {
    ...
    suspend fun findPopularSources(categoryId: Long, limit: Int): List<MangaSource> = db.getFavouritesDao().run {
        if (categoryId == 0L) {
    ...
    private fun observeOrder(categoryId: Long): Flow<ListSortOrder> = db.getFavouriteCategoriesDao().observe(categoryId)
        .filterNotNull()
        .map { x -> ListSortOrder(x.order, ListSortOrder.NEWEST) }
        .distinctUntilChanged()

    suspend fun getMostUpdatedCategories(limit: Int): List<FavouriteCategory> = db.getFavouriteCategoriesDao().getMostUpdatedCategories(limit).map {
        it.toFavouriteCategory()
    }
